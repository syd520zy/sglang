...
OUT="${1:-/workspace/sensenova-kv-results/$(date +%Y%m%d-%H%M%S)}"
mkdir -p "$OUT"
OUT="$(cd "$OUT" && pwd)"
{
    git log -1 --oneline
...
printf 'dtype,batch_size,prefix_length,exit_code\n' > "$OUT/status.csv"
failed=0
for dtype in bf16 fp16; do
    for batch in 1 2 4; do
        for prefix in 256 1024 4096; do
            name="${dtype}-b${batch}-p${prefix}"
            echo "Running $name"
            if python benchmark/kernels/attention/bench_sensenova_int8.py \
                --baseline sdpa --dtype "$dtype" --batch-size "$batch" \
                --prefix-length "$prefix" --image-tokens 1024 \
                --rounds 7 --sweep-kernel \
            ...
            printf '%s,%s,%s,%s\n' "$dtype" "$batch" "$prefix" "$code" >> "$OUT/status.csv"
        done
    done
done
echo "Results: $OUT"
